...
public class OneDimensionEventStatistics {
            ...
            //获取维度信息
                ...
                for (DimensionInformation dimensionInformation:dimensionInformations){
                    ...
                    for (WebsiteMessage websiteMessage:websiteMessages){
                        //获取对应维度信息的所有事件RDD
                        JavaRDD<Row> actionRDD = getActionRDD(sqlContext,websiteMessage.getDomain(),dimension,dimensionInformation);
                        //获取事件总数,没必要写到数据库，这个数据可以让后台自己去把所有事件加起来得到
                        Long totalEvent = actionRDD.count();
                        //获取维度相关用户，如果没有用户维度，则不用执行
                        JavaRDD<Long> userRDD = null;
                        if (dimension.getDimensionType().equals("user")||dimension.getDimensionType().equals("user_group")){
                            userRDD = getUserRDD(sqlContext,dimension,dimensionInformation,websiteMessage);
                        }
                        //获取网站的事件
                        ...
                        if (eventMessages.size()!=0){
                            for (EventMessage eventMessage:eventMessages){
                                ...
                                if (dimension.getDimensionType().equals("user")){
                                ...
                                //插入数据库中
//                                eventAnalysisDAO.insert(new EventAnalysis(eventMessage.getEventId(),dimension.getDimensionId(),dimensionInformation.getDimensionInformationId(),null,null,eventNum,userNum,ipNum,new Date()));
                            }
                        }
                    }
    ...
    private static JavaRDD<Long> getUserRDD(SQLContext sqlContext, Dimension dimension, DimensionInformation dimensionInformation, WebsiteMessage websiteMessage) {
    ...
    private static JavaRDD<Row> getActionRDD(SQLContext sqlContext,String domain, Dimension dimension,DimensionInformation dimensionInformation) {
        String sql = "select * "
                + "from user_action "
